...
if [ ! -s dftbp_band.dat ]; then
  echo "---------------------------------"     > msd.dat
  echo "Standard diviation (SD) = 9.999 [eV]" >> msd.dat
  echo "---------------------------------"    >> msd.dat
  echo "SDT = 9.999 [eV]"                     >> msd.dat
  echo "---------------------------------"    >> msd.dat
  echo "SDT = sqrt(sum(VDT)/n) "              >> msd.dat
  echo "VDT = (E(DFT)-E(DFTB))^2*exp(-|E(DFTB)-EF|/(3.0*kbT))" >> msd.dat
  echo "3*kbT = 9.999"                        >> msd.dat
  echo "---------------------------------"    >> msd.dat
  echo "ETA = 9.999 [eV]"                     >> msd.dat
  echo "ETA = sum(|E(DFT)-E(DFTB)|)/n"        >> msd.dat
  echo "---------------------------------"    >> msd.dat
else
  echo "---------------------------------"
