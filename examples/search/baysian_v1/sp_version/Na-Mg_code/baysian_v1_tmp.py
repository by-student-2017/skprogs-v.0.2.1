...
#---------------------------
hwb_den = 0.27 # search range [-x*hwb:+x*hwt]
hwt_den = 0.27 # search range [-x*hwb:+x*hwt]
#---------------------------
...
print("initial parameters:   x0  x1  x2  x3  x4  x5  x6  x8   x9"
  +"  x10  x7  x11  x12  x13  x14  x15")
print("initial parameters: ",x0,x1,x2,x3,x4,x5,x6,x8,x9,x10,x7,x11,x12,x13,x14,x15)
#x = [x0,x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,x13,x14,x15,x16] # spd
x = [x0,x1,x2,x3,x4,x5,x6,x8,x9,x10,x7,x11,x12,x13,x14,x15]
#------------------------------------------------

#------------------------------------------------
n_gene = 16 # number of parameters, number of individual +1
...
#---------------------------
# Slater-Type Orbitals of S: ylasts
...
#---------------------------
# Slater-Type Orbitals of P: ylastp
...
subprocess.run("echo \"#| iter | 1/target | x0 | x1 | x2 | x3 | x4 | x5 | x6 | x8 |"
  +" x9 | x10 | x14 | x7 | x11 | x12 | x13 | x15 | SDT | SDT(3kbT) |\" >> Evalute.txt", shell=True)
#------------------------------------------------
...
count = 0
#----------------------------------------------------------------------
#def descripter(x0,x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,x13,x14,x15,x16): # spd
def descripter(x0,x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,x13,x14,x15): # sp
  ...
  if fine_step == "no": # 0.2 and 0.02 step for sigma and sto, respectively.
    ...
    #--------- 0.02 step
    if int(x6*100+0.5) % 2 == 0:
      sx6 = "{:.2f}".format(x6)
    else:
      sx6 = "{:.2f}".format(x6+0.01)
    #--------- 0.02 step
    ...
    #--------- 0.02 step
    if int(x7*100+0.5) % 2 == 0:
      sx7 = "{:.2f}".format(x7)
    else:
      sx7 = "{:.2f}".format(x7+0.01)
    #--------- 0.02 step
  ...
  subprocess.run("echo No."+str(count)+": "+str(etav)
    ...
    +", "+sx2+", "+sx3 # S orbital
    +", "+sx4+", "+sx5 # P orbital
    +", "+sx6+", "+sx8+", "+sx9+", "+sx10+", "+sx14   # Slater-Type Orbitals of S
    +", "+sx7+", "+sx11+", "+sx12+", "+sx13+", "+sx15 # Slater-Type Orbitals of P
...
if os.path.exists("./logs.json"):
  ...
  optimizer.maximize(init_points=(n_gene*11), n_iter=(350*1)) # 600 cycles / 12 h (Note: It depends on the number of parameters and search range, but usually around 150 times is a good value (in n_gene*5 case). I set it to 600 just in case (from after I got home until the next morning).)
