...
print("initial parameters:   x0  x1  x2  x3  x4  x5  x6  x8  x9  x7  x11  x12")
print("initial parameters: ",x0,x1,x2,x3,x4,x5,x6,x8,x9,x7,x11,x12)
#x = [x0,x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,x13,x14,x15,x16] # spd
x = [x0,x1,x2,x3,x4,x5,x6,x8,x9,x7,x11,x12]
#------------------------------------------------

#------------------------------------------------
n_gene = 13 # number of parameters, number of individual +1
...
subprocess.run("echo \"#| iter | 1/target | x0 | x1 | x2 | x3 | x4 | x5 | x6 | x8 |"
  +" x9 | x7 | x11 | x12 | SDT | SDT(3kbT) |\" >> Evalute.txt", shell=True)
#------------------------------------------------
...
def descripter(x0,x1,x2,x3,x4,x5,x6,x8,x9,x7,x11,x12): # sp
  ...
  subprocess.run("echo No."+str(count)+": "+str(etav)
    ...
    +", "+sx2+", "+sx3 # S orbital
    +", "+sx4+", "+sx5 # P orbital
    +", "+sx6+", "+sx8+", "+sx9   # Slater-Type Orbitals of S
    +", "+sx7+", "+sx11+", "+sx12 # Slater-Type Orbitals of P
...
if os.path.exists("./logs.json"):
  ...
  optimizer.maximize(init_points=(n_gene*11), n_iter=(250*1)) # 600 cycles / 12 h (Note: It depends on the number of parameters and search range, but usually around 150 times is a good value (in n_gene*5 case). I set it to 600 just in case (from after I got home until the next morning).)
