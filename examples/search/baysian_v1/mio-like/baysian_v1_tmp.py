...
#---------------------------
hwb_den = 15.0 # search range [-x*hwb:+x*hwt]
hwt_den = 15.0 # search range [-x*hwb:+x*hwt]
#---------------------------
hwb_wav = 1.5 # search range [-x*hwb:+x*hwt]
hwt_wav = 2.5 # search range [-x*hwb:+x*hwt]
#---------------------------
...
if os.path.exists("./logs.json"):
  ...
  optimizer.maximize(init_points=(n_gene*30), n_iter=(200*1)) # 600 cycles / 12 h (Note: It depends on the number of parameters and search range, but usually around 150 times is a good value (in n_gene*5 case). I set it to 600 just in case (from after I got home until the next morning).)
