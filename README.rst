...
- In "STO-nG (https://www.basissetexchange.org/)" used in Gaussian etc., s and p orbitals are treated as SP. Therefore, regarding radial wave functions, s and p are often treated as SP. New Info: A tool (skexp.py) has recently been developed that uses skexp.py to set the STO basis by making all s, p, d, and f the same.
