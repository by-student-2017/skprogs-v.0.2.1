...
Although not mentioned much in papers, experimentally the interatomic distance is determined by the sum of the covalent bond radii of paired elements (for example, A and B) (Rcov(AB)=rcov(A)+rcov(B )) is known to be. Therefore, for the unitary system (A and A), it is natural that r0, which is related to potential confinement, is twice rcov.
