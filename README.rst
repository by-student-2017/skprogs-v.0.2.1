...
- Even on the same OS (for example, Ubuntu 22.04 LTS), calculations may not converge well on Windows WSL. In that case, it is a good idea to reduce the STO value by a few percentage points according to the instructions on the screen. The electronic structure changes slightly compared to Linux (reproduction of the electronic structure is often worse), but unnecessary unoccupied level bands tend to move away from the Fermi level, making it more versatile. 
- The .skf created on Linux also works with WSL on Windows, so there is no problem if you can create .skf, but you can check this Github to try out combinations of elements, or find parameters that are more suitable for the desired alloy. Users who want to create .
- (r/12)^(6) = const. => 1 = const. * (12/r)^(-6) : The -6 power attractive term of the Lennard-Jones potential refers to the dispersion force between two atoms, or dipole-dipole interaction.
- In the Lennard-Jones (LJ) potential, the −6th power attractive term is the dispersion force between two atoms. On the other hand, the repulsive force term to the −12th power is the repulsive force due to the overlap of the electron clouds. Since the exponent -12 is exactly the square of the -6 power, treating the density simga as 6 also leads to considering these effects. Since PBE, which is an exchange-correlation term, cannot handle dispersion forces (van der Waals forces, etc.) well, it is thought that the repulsive force of the electron cloud is mainly involved. 
...
- [AE1] [Alamode Example](https://github.com/by-student-2017/alamode-example) (see "plot_band.png" in every folders. I had high hopes for OC20, but the Si phonon dispersion ended up with very disappointing results. The reason I have not published these results in a paper is because I have high hopes for the Open Catalysts Project. I recently learned that the z-axis direction is not a periodic boundary condition. I wish they would have made it more widely known.)
...
In Figure 1, there are parts where the evaluation value deteriorates in large areas of r0, which shows that there is a risk that calculations will become unstable if the value is too large even in r0.
...
The success or failure of Bayesian optimization depends on the evaluation function and evaluation range settings. Complicating the evaluation function is not very effective. This is because it is not only more difficult to code, but also less versatile and makes it harder for humans to interpret and analyze the results. After experimenting with various evaluation functions, this study ultimately decided to adopt a simple evaluation function described in the literature.
...
As a result of changing the volume of a simple cubic with Nd by +/-20%, the electronic structure showed good results even when the interatomic distance became long.
On the other hand, when the interatomic distance is shortened, an unnecessary unoccupied level band appears in the valence band.
Therefore, it became clear that it is necessary to check whether the electronic structure is well reproduced in conditions where the interatomic distance becomes short, such as when bonding with elements with a small covalent bond radius or under ultrahigh pressure.
