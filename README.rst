...
- In the Lennard-Jones (LJ) potential, the −6th power attractive term is the dispersion force between two atoms. On the other hand, the repulsive force term to the −12th power is the repulsive force due to the overlap of the electron clouds. Since the exponent -12 is exactly the square of the -6 power, treating the density simga as 6 also leads to considering these effects. Since PBE, which is an exchange-correlation term, cannot handle dispersion forces well, it is thought that the repulsive force of the electron cloud is mainly involved. If we treat the exponent to the -6th power, we get 11^(-2)=3.31, which is a value close to the interatomic distance. The covalent radius of boron is close to 3.31/2 [bohr] = 0.87 [Angstrom].
