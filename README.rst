...
- Until now, accuracy similar to DFT was often required, but in an era where machine learning can be used, it is important to obtain a correlation rather than matching DFT. This is because it would be good if it could solve the weaknesses of machine learning, the need for huge amounts of data and the problem of extrapolation. DFTB+'s high transferability and fast calculations meet these requirements. Another attractive feature of Slater-Koster files and DFTB+ is that they are not black boxes. In addition, it is sufficient to perform highly accurate quantum chemical calculations and first-principles calculations using single-point calculations, and it goes without saying that DFTB+ and classical MD are important for structural optimization in the process up to that point. There are too many people in this world who have forgotten that.
